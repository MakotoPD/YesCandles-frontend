...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Get promotion code from request body
    const { promo_codes } = await readBody(event)
    if (!promo_codes) {
    ...
    throw createError({
      ...
      statusMessage: error.data?.message || error.message || 'Error applying promotion code',
    })
  }
})
